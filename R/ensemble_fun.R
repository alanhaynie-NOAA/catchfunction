ensemble_fun <- function(ABC.DATA,scenario,outputtype) {
  ...
  if (scenario == 1 | scenario == 2 | scenario == 3) {
    FISH.DATA <- ABC.DATA
    FISH.DATA$NETABC <- sum(ABC.DATA, na.rm=TRUE)
    FISH.DATA$NETABC.UB <- as.numeric(FISH.DATA$NETABC > 2.5e6)
    FISH.DATA$ABCboth <- FISH.DATA$ABC.BSAI.202 + FISH.DATA$ABC.BS.201
    FISH.DATA$ABCboth_ind <- as.numeric(FISH.DATA$ABC.BS.201 + FISH.DATA$ABC.BSAI.202 >= 1.5e6)
    FISH.DATA$ABCboth2 <- pmin(FISH.DATA$ABCboth,1.5e6)
    FISH.DATA$ABCpollock.ind = as.numeric(FISH.DATA$ABC.BS.201 > 1.25e6)
    FISH.DATA$ABCpollock.125 <- pmin(FISH.DATA$ABC.BSAI.201, 1.25e6)
  ...
  FISH.DATA$A82 <- 1
  FISH.DATA$newmngmnt <- 1 # new regs affecting management of atka, pollock, and pcod (in AI?)
  FISH.DATA$post2014 <- 1
